...
class RobotAPI:
    ...
    def navigate(self,
        ...
        assert(len(pose) > 2)
        url = self.prefix +\
            f'/open-rmf/kft_demos_fm/navigate/?robot_name={robot_name}' \
            f'&cmd_id={cmd_id}'
    ...
    def start_process(self,
        ...
        url = self.prefix +\
            f"/open-rmf/kft_demos_fm/start_task?robot_name={robot_name}" \
            f"&cmd_id={cmd_id}"
    ...
    def stop(self, robot_name: str, cmd_id: int):
        ...
        url = self.prefix +\
            f'/open-rmf/kft_demos_fm/stop_robot?robot_name={robot_name}' \
            f'&cmd_id={cmd_id}'
        try:
    ...
    def toggle_action(self, robot_name: str, toggle: bool):
        ...
        url = self.prefix +\
            f"/open-rmf/kft_demos_fm/toggle_action?robot_name={robot_name}"
        data = {'toggle': toggle}
        try:
            response = requests.post(url, timeout=self.timeout, json=data)
            response.raise_for_status()
    ...
    def data(self, robot_name=None):
        if robot_name is None:
            url = self.prefix + f'/open-rmf/kft_demos_fm/status/'
        else:
            url = self.prefix +\
                f'/open-rmf/kft_demos_fm/status?robot_name={robot_name}'
        try:
