...
from kft_demos_panel.dispatcher_client import DispatcherClient
from kft_demos_panel.rmf_msg_observer import AsyncRmfMsgObserver, RmfMsgType
...
# Note: Get Dashboard Config for each "World", specific to kft_demos impl
...
def main(args=None):
    ...
    if "kft_demos_API_SERVER_IP" in os.environ:
        server_ip = os.environ['kft_demos_API_SERVER_IP']
        print(f"Set Server IP to: {server_ip}")

    if "kft_demos_API_SERVER_PORT" in os.environ:
        port_num = int(os.environ['kft_demos_API_SERVER_PORT'])
    ...
    print(f"Starting kft_demos API Server: {server_ip}:{port_num}, "
