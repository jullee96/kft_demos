...
![](https://github.com/open-rmf/kft_demos/workflows/build/badge.svg)
![](https://github.com/open-rmf/kft_demos/workflows/style/badge.svg)
...
```bash
ros2 launch kft_demos_gz office.launch.xml server_uri:="ws://localhost:8000/_internal"
```
...
ros2 launch kft_demos_gz_classic office.launch.xml gazebo_version:=9

> To run the demos using Ignition instead of Gazebo, change the commands from `ros2 launch kft_demos_gz_classic [...]` to `ros2 launch kft_demos_gz [...]`
...
The [RMF panel](https://github.com/open-rmf/rmf-panel-js) is a web based dashboard for interacting with kft_demos. It allows users to send task requests to RMF and monitor the status of robots and submitted tasks. For more [details](kft_demos_panel/README.md).

There are two main modes of submitting tasks to Open-RMF via the Panel:

1. Submit a Task: Used to submit a single task.
2. Submit a List of Tasks: Used to submit a batch of tasks. A `.json` file containing a list of tasks may be loaded via the `Choose file` button. Some example files are found in `kft_demos_panel/task_lists`.
...
```
ros2 launch kft_demos_gz_classic office.launch.xml server_uri:="ws://localhost:7878"
```
...
ros2 launch kft_demos_gz_classic hotel.launch.xml

# Or, run with ignition simulator
ros2 launch kft_demos_gz hotel.launch.xml
```
...
```bash
ros2 run kft_demos_tasks dispatch_clean -cs clean_lobby --use_sim_time
ros2 run kft_demos_tasks dispatch_patrol -p restaurant  L3_master_suite -n 1 --use_sim_time
```
...
To submit a list of scheduled tasks via rmf web panel, load [hotel_tasks.json](https://github.com/open-rmf/kft_demos/blob/main/kft_demos_panel/task_lists/hotel_tasks.json),
...
```bash
source ~/kft_demos_ws/install/setup.bash
ros2 launch kft_demos_gz_classic office.launch.xml

# Or, run with ignition simulator
ros2 launch kft_demos_gz office.launch.xml
```

Now we will showcase 2 types of Tasks: **Delivery** and **Loop**

To send task requests, open kft_demos web panel: https://open-rmf.github.io/rmf-panel-js/
...
```bash
ros2 run kft_demos_tasks dispatch_patrol -p coe lounge -n 3 --use_sim_time
ros2 run kft_demos_tasks dispatch_delivery -p pantry -ph coke_dispenser -d hardware_2 -dh coke_ingestor --use_sim_time
```
...
To run a scenario with multiple task requests, load [office_tasks.json](https://github.com/open-rmf/kft_demos/blob/main/kft_demos_panel/task_lists/office_tasks.json) from `kft_demos_panel/task_lists` in the `Submit a list of tasks` section. This should populate the preview window with a list of tasks. Click submit and watch the demonstration unfold.
...
```bash
source ~/rmf_ws/install/setup.bash
ros2 launch kft_demos_gz_classic airport_terminal.launch.xml
```

Open [RMF Panel](https://open-rmf.github.io/rmf-panel-js/). Load the [airport_terminal_tasks.json](https://github.com/open-rmf/kft_demos/blob/main/kft_demos_panel/task_lists/airport_terminal_tasks.json) list and click submit to begin a collection of loop, delivery and cleaning tasks.

Or, submit `loop`, `delivery` or `clean` task via CLI:
```bash
ros2 run kft_demos_tasks dispatch_patrol -p s07 n12 -n 3 --use_sim_time
ros2 run kft_demos_tasks dispatch_delivery -p mopcart_pickup -ph mopcart_dispenser -d spill -dh mopcart_collector --use_sim_time
ros2 run kft_demos_tasks dispatch_clean -cs zone_3 --use_sim_time
```

To see crowd simulation in action, enable crowd sim by:
```bash
ros2 launch kft_demos_gz_classic airport_terminal.launch.xml use_crowdsim:=1
```
...
ros2 launch kft_demos_gz airport_terminal_caddy.launch.xml
```
...
```bash
source ~/rmf_ws/install/setup.bash
ros2 launch kft_demos_gz_classic clinic.launch.xml
```

Open [RMF Panel](https://open-rmf.github.io/rmf-panel-js/). Load the [clinic_tasks.json](https://github.com/open-rmf/kft_demos/blob/main/kft_demos_panel/task_lists/clinic_tasks.json) list and click submit to begin a collection of loop and delivery tasks.

Or, submit a task via CLI:
```bash
ros2 run kft_demos_tasks dispatch_patrol -p L1_left_nurse_center L2_right_nurse_center -n 5 --use_sim_time
ros2 run kft_demos_tasks dispatch_patrol -p L2_north_counter L1_right_nurse_center -n 5 --use_sim_time
```
...
```bash
source ~/rmf_ws/install/setup.bash
ros2 launch kft_demos_gz campus.launch.xml

ros2 run kft_demos_tasks  dispatch_patrol -p room_5 campus_4 -n 10 --use_sim_time
ros2 run kft_demos_tasks  dispatch_patrol -p campus_5 room_3 -n 10 --use_sim_time
ros2 run kft_demos_tasks  dispatch_patrol -p room_2 dead_end -n 10 --use_sim_time
```

#### RobotManager Integration
`fleet_robotmanager_mqtt_bridge` (see [kft_demos_bridges](https://github.com/open-rmf/kft_demos/tree/main/kft_demos_bridges/kft_demos_bridges)) can be used to publish robot locations, battery percentage and state to a `/robot/status/ROBOT-ID` websocket endpoint. An instance of RobotManager can be configured to subscribe to this server to receive json messages, which will in turn visualize the robots on RobotManager.
...
# Start the bridge
ros2 run kft_demos_bridges fleet_robotmanager_mqtt_bridge -y 31500 -x 22000
```
...
Open-RMF can also manage fleets whose API or fleet managers only offer pause and resume commands to control their robots. Such fleets are classified as `traffic_light`. To integrate a `traffic_light` fleet, users are expected to implement a `traffic_light` fleet adapter based on this [API](https://github.com/open-rmf/rmf_ros2/blob/main/rmf_fleet_adapter/include/rmf_fleet_adapter/agv/EasyTrafficLight.hpp). The `kft_demos` repository contains demonstrations of `traffic_light` fleets in various scenarios. A simplistic `mock_traffic_light` adapter is used in these demonstrations.

#### Triple-H scenario:
```bash
$ ros2 launch kft_demos_gz_classic triple_H.launch.xml
(new terminal) $ ros2 launch kft_demos the_pedigree.launch.xml
```
#### Battle Royale Scenario:

```bash
$ ros2 launch kft_demos_gz_classic battle_royale.launch.xml
(new terminal) $ ros2 launch kft_demos battle_go.launch.xml
```

#### Office Scenario:
Note that `tinyRobot1` is a standard "full control" robot, while `tinyRobot2` "traffic light" robot.
```bash
$ ros2 launch kft_demos_gz_classic office_mock_traffic_light.launch.xml
(new terminal) $ ros2 launch kft_demos office_traffic_light_test.launch.xml
```

### Additional Features
 - **Flexible Tasks Scripts**
   For more [details](kft_demos_tasks/README.md).
 ...
 - **lift watchdog**
   ...
   - Command lines:
    ...
    ros2 launch kft_demos_gz_classic hotel.launch.xml enable_experimental_lift_watchdog:=1

    ## On a separate terminal, set lift as crowded
    ros2 launch kft_demos experimental_crowded_lift.launch.xml

    # Dispatch robot from level1 to level3, robot will wait in front of the lift cabin
    ros2 run kft_demos_tasks dispatch_patrol -p L3_room1  L3_room1 -n 1 --use_sim_time

    # Lift is cleared. Give robot the permission to enter the lift
    ros2 launch kft_demos experimental_clear_lift.launch.xml
    ```
